...
async function cargarHerramientasDesdeBD() { //async ya que va a realizar tareas que llevan tiempo
  //esta funcion es un puente entre la bd y lo que ve el usuario
  //su objetivo es traer la lista actualizada de herramientas desde el servidor
  try {
    //try es un bloque que funciona como red de seguridad y si algo falla salta al 
    //bloque catch para manejar el error en lugar de que se rompa todo
    const respuesta = await fetch('api/obtener_herramientas.php');
    //se hace una llamada al archivo backend, se usa await para una pausa a la ejecucion
    //hasta que se reciba la respuesta del servidor 

    //una vez que el servidor haya respondido
    //se verifica si la respuesta es exitosa con el ".ok" que viene de fetch 
    if (!respuesta.ok) {
      //si no es exitosa se lanza un error con "Throw new error" y nos manda directamente
      //al catch para que maneje el error
      throw new Error(`Error HTTP: ${respuesta.status}`);
    }

    // guardamos esta lista en la variable global "herramientas"
    herramientas = await respuesta.json();

    // Llama a las funciones de renderizado
    renderHerramientas(herramientas);
    actualizarTablaHerramientas(); // <-- Actualiza la tabla de inventario también
    actualizarDashboard();
...
function actualizarTablaHerramientas() {
  ...
  herramientas.forEach(h => {
    ...
    // Limpiar descripción de detalles del Acta para la tabla
...
// Genera un acta para una herramienta específica (rota/desaparecida)
// Flujo de gestión de acta (similar a "Gestionar" en No Devueltas)
async function gestionarActa(idHerramienta, edicion = false) {
  ...
  // Resumen previo de confirmación
  ...
  // Persistir agregando el detalle del acta a la descripción; NO se cambia el estado
  try {
    // Reemplazar acta previa si existiera y agregar la nueva
...
// Abrir modal en modo edición e impresión combinados
...
// Mantengo la firma anterior por compatibilidad; redirige al nuevo flujo
async function generarActa(idHerramienta) {
  return gestionarActa(idHerramienta);
}

// Guardar desde modal (y opcionalmente imprimir)
